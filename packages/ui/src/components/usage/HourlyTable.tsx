        ...
        <div className="flex gap-1">
          {(['hour', 'provider', 'model'] as DrillDownLevel[]).map(level => (
            <button key={level} onClick={() => { setDrillDownLevel(level); setExpandedRows(new Set()); }}
              className={`px-3 py-1 text-sm rounded-md transition-colors ${drillDownLevel === level ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'bg-slate-800/50 hover:bg-slate-700 text-slate-300 border border-slate-700/50'}`}>
              {t(`usage.${level}`)}
            </button>
          ))}
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-700/50 bg-slate-800/30">
              <th className="p-2 text-left text-slate-400 font-semibold">{firstColLabel}</th>
              <th className="p-2 text-right text-slate-400 font-semibold">{t('usage.request_count')}</th>
              <th className="p-2 text-right text-slate-400 font-semibold">{t('usage.input_tokens')}</th>
              <th className="p-2 text-right text-slate-400 font-semibold">{t('usage.cache_hit')}</th>
              <th className="p-2 text-right text-slate-400 font-semibold">{t('usage.reasoning')}</th>
              <th className="p-2 text-right text-slate-400 font-semibold">{t('usage.output_tokens')}</th>
              <th className="p-2 text-right text-slate-400 font-semibold">{t('usage.consumed_tokens')}</th>
              <th className="p-2 text-right text-slate-400 font-semibold">{t('usage.avg_latency')}</th>
              <th className="p-2 text-right text-slate-400 font-semibold">{t('usage.avg_speed')}</th>
            </tr>
          </thead>
          ...
          <tfoot>
            <tr className="border-t-2 border-slate-700/50 font-semibold bg-slate-800/50 text-slate-200">
              <td className="p-2">{t('usage.total')}</td>
              <td className="p-2 text-right">{totals.requests.toLocaleString()}</td>
              <td className="p-2 text-right font-mono text-cyan-300">{formatTokens(totals.inputTokens)}</td>
              <td className="p-2 text-right font-mono text-xs text-emerald-400/80">{formatCacheHit(totals.cacheReadTokens)}</td>
              <td className="p-2 text-right">
                {(totals.reasoningTokens ?? 0) > 0 ? <span className="text-amber-400">{formatTokens(totals.reasoningTokens)}</span> : '-'}
              </td>
              <td className="p-2 text-right font-mono text-fuchsia-300">{formatTokens(totals.outputTokens)}</td>
